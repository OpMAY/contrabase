...
@Slf4j
public class WorkApplyJsonObjectTypeHandler<T> extends BaseTypeHandler<T> {

    private Class<T> type;

    public WorkApplyJsonObjectTypeHandler(Class<T> type) {
    ...
    private T convertToObject(String jsonString) {
        if (jsonString != null) {
            try {
                return (T) new Gson().fromJson(jsonString, WorkApply.class);
            } catch (Exception e) {
